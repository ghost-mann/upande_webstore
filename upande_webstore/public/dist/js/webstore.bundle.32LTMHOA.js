(() => {
  ...
  (() => {
    ...
    document.addEventListener("click", async (event) => {
      ...
      if (add) {
        const card = add.closest(".ws-card");
        const qtyInput = (card == null ? void 0 : card.querySelector("[data-webstore-qty]")) || document.getElementById("webstore-qty");
    ...
    async function switchCurrency(select) {
    ...
    document.addEventListener("change", (event) => {
      const target = event.target;
      if (target.matches("select.webstore-attribute")) {
        onAttributeChange();
        return;
      }
      if (target.matches("[data-ws-currency-select]")) {
